...
function Creategroup() {
  ...
  useEffect(()=>{
  axios.post(`${BASE_URL}/user/finduser`,{name:findUsers},{withCredentials:true})
  ...
  function createGroup(){
    ...
    try{
      axios.post(`${BASE_URL}/user/creategroup`,{
      users:users,
      name:groupName,
      file:file
  ...
  return (
    ...
    <div className='w-[100%] h-[20%] flex flex-col gap-3 justify-end items-center'>
      <input placeholder='Enter Group Name' className='p-[1%] rounded-xl w-[60%] outline-none border-2' value={groupName} onChange={(e)=>setGroupName(e.target.value)}/>
      <input placeholder='Search Users'className='p-[1%] rounded-xl w-[60%] outline-none border-2' value={findUsers} onChange={(e)=>setFindUsers(e.target.value)}/>
        ...
        user.length>0?(
          user.map((users)=>{
            if(users._id!==localStorage.getItem('id')){
              return <div className='cursor-pointer flex items-center gap-2 my-[1%] text-xl bg-gray-100 border-2 shadow-lg rounded-xl ' onClick={()=>Adduser(users._id)}>
              <div className='w-[30%] py-1'>
                {
                  users.imageUrl!==null?(
                    <img src={users.imageUrl} className='h-16 mx-auto  rounded-full w-16' alt=""/>
                  ):(
                    <img src={Default} className='h-16 mx-auto  rounded-full w-16' alt=""/>
                  )
                }
              </div>  
